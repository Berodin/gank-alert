...
def test_fetch_current_labels_returns_labels():
    respx.get("https://zkillboard.com/api/killID/137021750/").mock(
...
def test_fetch_current_labels_empty_response_returns_empty_list():
    respx.get("https://zkillboard.com/api/killID/999999999/").mock(return_value=httpx.Response(200, json=[]))
...
def test_fetch_current_labels_http_error_returns_empty_list():
    respx.get("https://zkillboard.com/api/killID/1/").mock(return_value=httpx.Response(500))
...
def test_fetch_current_labels_network_error_returns_empty_list():
    respx.get("https://zkillboard.com/api/killID/1/").mock(side_effect=httpx.ConnectError("boom"))
